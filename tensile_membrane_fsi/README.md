...
| Membrane flutter (3-D) | Flow past the membrane |
|---|---|
| ![membrane flutter](docs/membrane_flutter.gif) | ![flow past membrane](docs/flow_past_membrane.gif) |

*(generated by `python examples/run_fsi_animation.py`)*
...
# animated GIFs: 3D membrane flutter + flow past the membrane
python examples/run_fsi_animation.py --t-end 1.0 --save-interval 2
...
`examples/run_fsi_animation.py` runs the coupled transient and writes two GIFs to
`output/` from the **real timesteps** of the simulation:

- `membrane_flutter.gif` — 3-D membrane surface moving/fluttering, coloured by displacement.
- `flow_past_membrane.gif` — mid-plane `|U|` field with the membrane cross-section overlaid.

The reusable helpers are `animate_membrane_3d` and `animate_flow_slice` in
`src/utils/viz.py`.
...
- `membrane_flutter.gif` / `flow_past_membrane.gif` — animations (via `run_fsi_animation.py`)
